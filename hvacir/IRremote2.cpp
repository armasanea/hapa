...
int MATCH(int measured, int desired) {
  print("Testing: ");
  print(TICKS_LOW(desired), DEC);
  print(" <= ");
  print(measured, DEC);
  print(" <= ");
  printf(TICKS_HIGH(desired), DEC);
  return measured >= TICKS_LOW(desired) && measured <= TICKS_HIGH(desired);
}

int MATCH_MARK(int measured_ticks, int desired_us) {
  print("Testing mark ");
  print(measured_ticks * USECPERTICK, DEC);
  print(" vs ");
  print(desired_us, DEC);
  print(": ");
  print(TICKS_LOW(desired_us + MARK_EXCESS), DEC);
  print(" <= ");
  print(measured_ticks, DEC);
  print(" <= ");
  printf(TICKS_HIGH(desired_us + MARK_EXCESS), DEC);
...
//#define  HVAC_MITSUBISHI_DEBUG;  // Un comment to access DEBUG information through Serial Interface
  ...
  switch (HVAC_FanMode)
  {
    case FAN_SPEED_1:       data[9] = (byte) 0x00000001; break;
    case FAN_SPEED_2:       data[9] = (byte) 0x00000010; break;
    case FAN_SPEED_3:       data[9] = (byte) 0x00000011; break;
    case FAN_SPEED_4:       data[9] = (byte) 0x00000100; break;
    case FAN_SPEED_5:       data[9] = (byte) 0x00000100; break; //No FAN speed 5 for MITSUBISHI so it is consider as Speed 4
    case FAN_SPEED_AUTO:    data[9] = (byte) 0x10000000; break;
    case FAN_SPEED_SILENT:  data[9] = (byte) 0x00000101; break;
    default: break;
  }

  switch (HVAC_VanneMode)
  {
    case VANNE_AUTO:        data[9] = (byte) data[9] | 0x01000000; break;
    case VANNE_H1:          data[9] = (byte) data[9] | 0x01001000; break;
    case VANNE_H2:          data[9] = (byte) data[9] | 0x01010000; break;
    case VANNE_H3:          data[9] = (byte) data[9] | 0x01011000; break;
    case VANNE_H4:          data[9] = (byte) data[9] | 0x01100000; break;
    case VANNE_H5:          data[9] = (byte) data[9] | 0x01101000; break;
    case VANNE_AUTO_MOVE:   data[9] = (byte) data[9] | 0x01111000; break;
    default: break;
  }
  ...
  printf("Packet to send: ");
  for (i = 0; i < 18; i++) {
    print("_"); print(data[i], HEX);
  }
  printf(".");
  for (i = 0; i < 18; i++) {
    print(data[i], BIN); print(" ");
  }
  printf(".");
#endif
...
long IRrecv::decodeMitsubishi(decode_results *results) {
  // print("?!? decoding Mitsubishi:");print(irparams.rawlen); print(" want "); printf( 2 * MITSUBISHI_BITS + 2);
  long data = 0;
  ...
  */
  ...
  while (offset + 1 < irparams.rawlen) {
    if (MATCH_MARK(results->rawbuf[offset], MITSUBISHI_ONE_MARK)) {
      ...
      // printf("A"); printf(offset); printf(results->rawbuf[offset]);
      return ERR;
    }
    offset++;
    if (!MATCH_SPACE(results->rawbuf[offset], MITSUBISHI_HDR_SPACE)) {
      // printf("B"); printf(offset); printf(results->rawbuf[offset]);
      break;
    }
    offset++;
  }
